...
def register(request):
    ...
    if serializer.is_valid():
        email_match = User.objects.filter(
            email__exact=serializer.data.get('email')
        )
        username_match = User.objects.filter(
            username__exact=serializer.data.get('username')
        )
        password = serializer.data.get('password')
        if email_match.exists() is True:
            response = JsonResponse(
                {'registered': False, 'err_msg': 'Email already exists!'}
            )
        elif username_match.exists() is True:
            response = JsonResponse(
                {'registered': False, 'err_msg': 'Username already exists!'}
            )
        else:
            ...
            user.save()
            response = JsonResponse(
                {'registered': True, 'success_msg': 'User Registration Success'}
            )
...
def login(request):
    ...
    if email_match.exists():
        ...
        actual_password = username_match.values_list(
            'password',
            flat=True
        ).get()
        password_match = check_password(entered_password, actual_password)
